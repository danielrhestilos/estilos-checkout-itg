...
async function renderSocialSelling() {
  await fetch('/api/checkout/pub/orderForm')
    .then((respose) => respose.json())
    .then((result) => {
      socialSellingContent(result.orderFormId)
    })
}

function socialSellingContent(sendId: string){
  // Verificar si ya se pintó el contenedor
  const existingContainer = document.querySelector('.containerShareCart');
  ...
  container.className = 'containerShareCart';
  ...
  const summaryTemplateHolder = document.querySelector('.clearfix.pull-right.cart-links.cart-links-bottom.hide');

  if (summaryTemplateHolder) {
      summaryTemplateHolder?.insertBefore(container, summaryTemplateHolder.firstChild);
  }
}
...
export default renderSocialSelling
